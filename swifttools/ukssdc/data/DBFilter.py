...
class filter:
    ...
    def __init__(self, filterDef, metadata=None):
        ...
        metadata : Table metadata; used to check filter is valid.
        ...
        for s in suffs:
            ...
            # Value
            v = f"val{s}"
            if (numArgs > 0) and (v not in filterDef):
        ...
        if (metadata is not None) and (not self.isValid(metadata)):
            self._data = None
            raise RuntimeError(f"Cannot create filter, {self._data['colName']} is not a valid column.")
    ...
    def isValid(self, metadata):
        ...
        metadata : pandas.DataFrame The table metadata

        Return
        ------

        bool - Whether or not the filter is valid
        """
